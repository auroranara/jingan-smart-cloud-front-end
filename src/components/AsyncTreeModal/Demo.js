...
export default class Text extends PureComponent {
  state = {
    tree: [
      { title: 'Expand to load', key: 'a', children: [
        { title: 'Expand to load', key: 'd', parentIds: '0,a' },
        { title: 'Tree Node', key: 'e', children: [
  ...
  handleLoadData = (data, callback) => {
    ...
    setTimeout(() => {
      const list = [
        { title: 'Child Node', key: `${+new Date()}`, parentIds: `${parentIds},${key}`, isLeaf: Math.random() > 0.5  },
        { title: 'Child Node', key: `${+new Date()+1}`, parentIds: `${parentIds},${key}`, isLeaf: Math.random() > 0.5 },
      ];
      // callback必须调用
      callback(list);
  ...
  render() {
    ...
    return (
      <Card>
        <Button type="primary" onClick={this.handleShowModal}>显示弹出框</Button>
        <AsyncTreeModal
          visible={visible}
