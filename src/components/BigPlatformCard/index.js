...
export default class BigPlatformCard extends PureComponent {
  ...
  renderFields(fieldsValue=this.getFieldsValue()) {
    return this.FIELDS.map(({ label, key, render, hidden, ...rest }) => !(hidden && hidden(fieldsValue)) && (this.renderField({ label: typeof label === 'function' ? label(fieldsValue) : label, value: render ? render(fieldsValue) : fieldsValue[key], ...rest })));
  }
