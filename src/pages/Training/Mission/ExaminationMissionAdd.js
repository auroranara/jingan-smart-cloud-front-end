...
import { Card, Form, Button, Row, Col, Input, Checkbox, DatePicker, AutoComplete, Transfer, Modal, Table, Icon, message, Tag } from 'antd';
...
export default class ExaminationMissionAdd extends PureComponent {
  ...
  handleSubmit = () => {
    ...
    validateFields((errors, values) => {
      if (!errors) {
        ...
        const { timeRange: [start, end], paperId, ...others } = values
        const payload = {
          ...others,
  ...
  handleConfirmPaper = () => {
    ...
    if (!selectedRows || selectedRows.length === 0) message.error('请先选择试卷')
  ...
  handleConfirmStudents = () => {
    ...
    const newStudents = list.reduce((arr, { studentId, name }) => {
      return targetKeys.includes(studentId) ? [
        ...arr,
        { studentId, name },
      ] : arr
    }, [])
  ...
  renderSelectStudents = () => {
    ...
    return (
      <Modal
        ...
        width={800}
        onCancel={() => { this.setState({ studentsModalVisible: false }) }}
            ...
            <Row>
              <Form.Item label="考试规则" {...formItemLayout}>
                {getFieldDecorator('arrRuleType', {
                  rules: [
                    { required: true, message: '请选择考试规则', type: 'array' },
                  ],
                })(
                  <Checkbox.Group>
                    <Row>
                      <Col span={24}><Checkbox value={'1'} disabled={arrRuleType.includes('2') || arrRuleType.includes('3')}>统一顺序</Checkbox></Col>
                      <Col span={24}><Checkbox value={'2'} disabled={arrRuleType.includes('1')}>选项随机</Checkbox></Col>
                      <Col span={24}><Checkbox value={'3'} disabled={arrRuleType.includes('1')}>顺序随机</Checkbox></Col>
                    </Row>
                  </Checkbox.Group>
                )}
              </Form.Item>
            ...
            <Row>
              <Form.Item label="考试期限" {...formItemLayout}>
                {getFieldDecorator('timeRange', {
                  rules: [
                    { required: true, message: '请选择考试期限', type: 'array' },
                  ],
                })(
                  <RangePicker
            ...
            <Row>
              <Form.Item label="选择试卷" {...formItemLayout}>
                {getFieldDecorator('paperId', {
                  rules: [
                    { required: true, message: '请选择试卷', type: 'object' },
                  ],
                })(
                  <AutoComplete
                    labelInValue
                  ...
                  >
                    <Input
                      suffix={(<Button type="primary" onClick={this.handleViewPaperModal}>选择</Button>)}
                    />
                  </AutoComplete>
                )}
              </Form.Item>
            ...
            <Row>
              <Form.Item label="考试人员" {...formItemLayout}>
                {getFieldDecorator('students', {
                  rules: [
                    { required: true, message: '请选择考试人员', type: 'array' },
                  ],
                })(
                  <Fragment>
                    <Icon onClick={this.handleViewStudentsModal} type="search" className={styles.iconSelectStudents} />
                    <div className={styles.studentsContainer}>
                      {students.map(item => (
                        <Tag closable onClose={() => this.handleDeleteStudent(item.studentId)} color="#108ee9" key={item.studentId}>{item.name}</Tag>
                      ))}
                    </div>
                  </Fragment>
                )}
