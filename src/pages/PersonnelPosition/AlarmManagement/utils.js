...
export const CK_OPTIONS = ['越界', '长时间逗留', '超员', '缺员'].map((label, i) => ({
  label,
  value: CK_VALUES[i],
}));
...
export function handleInitFormValues(values, selected, props) {
  return selected.reduce((prev, next) => [...prev, ...props[next]], []).reduce((prev, next) => {
    if (next === 'canEnterUsers') prev[next] = values[next].map(({ cardId }) => cardId);
    else prev[next] = String(values[next]); // 原来是数字，直接传到Form中的Input，提交的时候不会报错，但是validateFields中的回调函数也不会被调用
    return prev;
  }, {});
}

export function getRangeMsg(min, max) {
  if (min && max) return `(下级区域设定的值)${min} ≤ n ≤ ${max}(上级区域设定的值)`;
  else if (min) return `n ≥ ${min}(下级区域设定的值)`;
...
export function getJSONProp(json, prop) {
  if (isJSON(json)) return JSON.parse(json)[prop];
}
