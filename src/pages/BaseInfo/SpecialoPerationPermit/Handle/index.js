...
export default class SpecialoPerationPermitHandle extends PureComponent {
  ...
  renderForm = () => {
    ...
    const { frontPhoto, backPhoto, forntLoading } = this.state
    const uploadButton = (
      <div>
        <Icon type={forntLoading ? 'loading' : 'plus'} />
        <div>Upload</div>
      </div>
    );
    return (
          ...
          <FormItem label="操作证正面扫描件" {...formItemLayout}>
            {getFieldDecorator('frontPhoto')(
              <Fragment>
                ...
                >
                  {frontPhoto ? <img src={frontPhoto} alt="avatar" style={{ width: '100%' }} /> : uploadButton}
                </Upload>
              </Fragment>
            )}
          </FormItem>
            ...
            {getFieldDecorator('backPhoto')(
              <Fragment>
                <Upload
                  ...
                  onChange={this.handleFrontChange}
                >
                  {backPhoto ? <img src={backPhoto} alt="avatar" style={{ width: '100%' }} /> : uploadButton}
                </Upload>
              </Fragment>
            )}
          </FormItem>
