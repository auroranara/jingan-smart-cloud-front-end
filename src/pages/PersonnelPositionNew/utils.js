...
export function getSrc(type, ip, buildId, secret, appId, trackSn) {
  return `http://${ip}/js/tunnel.html?to=${type}&buildId=${buildId || BUILD_ID}&appid=${appId || APP_ID}&secret=${secret}&wh=false${trackSn === undefined ? '' : `&trackSn=${trackSn}`}`;
