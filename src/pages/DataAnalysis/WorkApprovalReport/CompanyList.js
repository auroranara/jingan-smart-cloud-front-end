...
import { hasAuthority, AuthLink, ERROR_MSG } from '@/utils/customAuth';
...
const {
  dataAnalysis: {
    workApprovalReport: { workApprovalList: workApprovalListCode },
  },
} = codes;
...
export default class CompanyList extends PureComponent {
  componentDidMount() {
    const {
      user: {
        currentUser: { unitType, companyName, companyId },
      },
    } = this.props;
        ...
        <Row gutter={16}>
          <Col {...colWrapper}>
            <Form.Item {...formItemStyle}>
              {getFieldDecorator('name')(<Input placeholder="单位名称" />)}
            </Form.Item>
          </Col>
          <Col {...colWrapper}>
            <Form.Item {...formItemStyle}>
              ...
              <List.Item key={id}>
                ...
                >
                  <Card
                    ...
                    style={{ cursor: viewListAuth ? 'pointer' : 'inherit' }}
                  >
                    <Ellipsis tooltip className={styles.ellipsisText} lines={1}>
                      地址：
                      {practicalAddressLabel || '暂无信息'}
                    </Ellipsis>
                    <Ellipsis tooltip className={styles.ellipsisText} lines={1}>
                      行业类别：
