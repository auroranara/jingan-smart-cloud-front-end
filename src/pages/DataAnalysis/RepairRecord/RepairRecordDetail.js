...
export default class RepairRecordDetail extends PureComponent {
  ...
  renderFormItem = (item, realStatus, value, content = value) => (
    <FormItem key={item.key} label={item.label} {...formItemLayout}>
      ...
      (realStatus !== '已处理' &&
        ...
        <span className={styles.formContent}>{content}</span>
      )}
    </FormItem>
  );
  ...
  render() {
    ...
    const statusLogo =
      (realStatus === '已处理' && processed) ||
      (realStatus === '处理中' && processing) ||
      (realStatus === '待处理' && toBeProcessed) ||
      null;
    return (
      <PageHeaderLayout title={title} breadcrumbList={breadcrumbList}>
        <Spin spinning={loading} delay={200}>
          <Card title="报修内容" className={styles.RepairRecordDetailCard}>
            <Form>{this.renderInfo(reportInfo)}</Form>
            <div className={styles.statusLogo} style={{ backgroundImage: `url(${statusLogo})` }} />
          </Card>
          <Card
            title="维修内容"
            style={{ marginTop: '10px' }}
            className={styles.RepairRecordDetailCard}
          >
            <Form>{this.renderInfo(repairInfo)}</Form>
          </Card>
        </Spin>
        <Lightbox
          images={imageFiles}
