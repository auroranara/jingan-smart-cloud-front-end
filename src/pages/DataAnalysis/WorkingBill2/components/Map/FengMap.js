...
export default class Map extends React.Component {
  ...
  setModelColor(groupId, points, color) {
    ...
    const arrayList = models
      ...
      .map(model => {
        ...
        return { buildingId: model.FID, points: model.mapCoord };
      });
    return arrayList;
    // this.props.getBuilding && this.props.getBuilding(arrayList, 1);
  }

  setModelDefault(groupId, points) {
    // 默认gid为1
    const models = map.getDatasByAlias(groupId, 'model');
    models.filter(({ mapCoord }) => isPointInPolygon(mapCoord, points)).map(model => {
  ...
  handleFinishDraw = () => {
    ...
    const groupId = Points.map(item => item.groupID)[0];
    map.clearLineMark();
    PolygonMarker = this.drawPolygon(groupId, Points, currColor);
    // const models = this.setModelColor(groupId, Points, currColor);
    const group = map.getFMGroup(groupId);
    const layerImg = group.getOrCreateLayer('imageMarker');
    ...
    // handleChangeMapSelect && handleChangeMapSelect(Points, []);
  };
  ...
  render() {
    const { style, readonly, mode } = this.props;
    const isNotDetail = mode !== 'detail';

    return (
      <div className={styles.container} style={style}>
        {isNotDetail && this.renderDrawBtns()}
        <div className={styles.map} id="fengMap" />
      </div>
    );
  }
}
