...
@Form.create()
export default class GridPointManage extends PureComponent {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    ...
    // 获取单位列表
    dispatch({
      type: 'riskPointManage/fetchGridCompanyList',
      payload: {
        pageSize,
        pageNum: 1,
      },
  ...
  //   dispatch(
  //     routerRedux.push(
  //       `/risk-control/grid-point-manage/grid-point-List/${id}?companyId=${id}&&companyName=${name}`
  //     )
  //   );
  // };
  ...
  handleLoadMore = () => {
    ...
    // 请求数据
    this.props.dispatch({
      type: 'riskPointManage/appendCompanyGridList',
      payload: {
        pageSize,
        pageNum: pageNum + 1,
        ...this.formData,
              ...
              <List.Item key={companyId}>
                <Card title={companyName} className={styles.card}>
                  <Row>
                    ...
                    {hasAuthority(codesMap.riskControl.gridPointManage.view, codes) ? (
                      <Col span={8} style={{ cursor: 'pointer' }}>
                        <Link
                          to={`/risk-control/grid-point-manage/grid-point-List/${companyId}?companyId=${companyId}&&companyName=${companyName}`}
                          target="_blank"
                        >
