...
export default class HistoryRecord extends PureComponent {
  ...
  renderQuery() {
    const {
      ...
      fireAlarm: { deviceCodes, dictDataList },
    } = this.props;
    return (
        ...
        <Form layout="inline">
          <FormItem>
            {getFieldDecorator('alertStatus')(
              <Select allowClear placeholder="警情状态" style={{ width: 130 }}>
          ...
          <FormItem>
            {getFieldDecorator('deviceCode')(
              <Select allowClear placeholder="主机编号" style={{ width: 130 }}>
          ...
          <FormItem>
            {getFieldDecorator('specificLocation')(
              <Input placeholder="请输入具体位置" style={{ width: 150 }} />
            )}
          </FormItem>
          <FormItem>
          ...
          <FormItem style={{ float: 'right' }}>
            <Button type="primary" onClick={this.handleQueryHistories}>
              搜索
            </Button>
          </FormItem>
        </Form>
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        align: 'center',
        render: (val, rows) => (
          <a style={{ width: '90px' }} onClick={() => this.handleViewDetail(rows)}>
            查看
          </a>
        ),
      },
        ...
        >
          <Form layout="vertical" style={{ textAlign: 'left', marginLeft: '30px' }}>
            ...
            <FormItem {...formItemLayout} label="发生时间：">
              {moment(historyDetail.time).format('YYYY-MM-DD HH:mm:ss')}
            </FormItem>
            <FormItem {...formItemLayout} label="主机编号：">
              {historyDetail.code}
            </FormItem>
            <FormItem {...formItemLayout} label="回路故障号：">
              {historyDetail.failureCode}
            </FormItem>
            <FormItem {...formItemLayout} label="设施部件类型：">
              {historyDetail.type}
            </FormItem>
            <FormItem {...formItemLayout} label="具体位置：">
              {historyDetail.position}
            </FormItem>
