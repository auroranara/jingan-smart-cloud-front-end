...
const GET_DETAIL = 'emergencyManagement/fetchEquipmentMaintDetail';
const ADD = 'emergencyManagement/addEquipmentMaint';
const EDIT = 'emergencyManagement/editEquipmentMaint';
...
export default class MaintEdit extends Component {
      ...
      if (type !== 'maintAdd') {
        ...
        getDetail &&
          getDetail({ id }, (success, data) => {
            if (success) {
              const { maintDate, maintPerson, maintInfo, maintEstimate, maintFileList } =
                data || {};
              this.form &&
              ...
              this.setState({
                fileList: maintFileList.map(({ id, dbUrl, webUrl, fileName }, index) => ({
                  uid: id || index,
                  fileName,
                  status: 'done',
                  name: fileName,
  ...
  getBreadcrumbList = title => {
    ...
    return [
      ...
      { title: '维保记录', name: '维保记录', href: `${PATH}/${equipId}/list` },
      { title, name: title },
    ];
  };
    ...
    validateFieldsAndScroll((errors, values) => {
      if (!errors) {
        ...
        const payload = {
          id,
          equipId,
          maintFileList: fileList.map(({ name, url, dbUrl }) => ({
            fileName: name,
            webUrl: url,
            dbUrl,
            name,
