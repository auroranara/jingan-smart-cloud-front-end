...
const GET_DETAIL = 'emergencyManagement/fetchEquipmentCheckDetail';
const ADD = 'emergencyManagement/addEquipmentCheck';
const EDIT = 'emergencyManagement/editEquipmentCheck';
...
export default class CheckEdit extends Component {
      ...
      if (type !== 'checkAdd') {
        ...
        getDetail &&
          getDetail({ id }, (success, data) => {
            if (success) {
              const { checkDate, checkPerson, checkInfo, checkEstimate, checkFileList } =
                data || {};
              this.form &&
              ...
              this.setState({
                fileList: checkFileList.map(({ id, dbUrl, webUrl, fileName }, index) => ({
                  uid: id || index,
                  fileName,
                  status: 'done',
                  name: fileName,
  ...
  getBreadcrumbList = title => {
    ...
    return [
      ...
      { title: '检查记录', name: '检查记录', href: `${PATH}/${equipId}/list` },
      { title, name: title },
    ];
  };
