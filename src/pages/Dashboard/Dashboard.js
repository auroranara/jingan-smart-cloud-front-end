...
export default class Dashboard extends PureComponent {
  ...
  render() {
    ...
    const imgWrapper = (safetyProduction === 1 && fireService === 1 && [safeItem, fireItem, safeItem, fireItem, safeItem, fireItem])
      || (safetyProduction === 1 && fireService === 0 && [safeItem, safeItem, safeItem, safeItem])
      || (safetyProduction === 0 && fireService === 1 && [fireItem, fireItem, fireItem, fireItem])
      || []
    ...
    return (
      <div className={styles.carouselDemoWrapper}>
        <Carousel3d className={styles.carouselDemo} onChange={this.handleChange} childMaxLength={maxLength}>
          {children}
        </Carousel3d>
      </div>
    )
