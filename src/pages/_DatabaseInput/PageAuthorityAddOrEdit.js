  ...
  render() {
    ...
    return (
      <PageHeaderLayout
        ...
        // }
        action={
          <Button type="primary" onClick={e => router.push('/database-input/pageAuthority/index')}>
            返回
          </Button>
        }
      >
        <Card>
          <Form onSubmit={this.handleSubmit}>
            ...
            <FormItem label="中文名字(参考)" {...formItemLayout}>
              {getFieldDecorator('zName', {})(<Input placeholder="请输入中文名称(参考)" />)}
            </FormItem>
            <FormItem label="中文名字(显示)" {...formItemLayout}>
              {getFieldDecorator('showZName', {
                rules: [{ required: true, message: '请输入中文名称(显示)' }],
              })(<Input placeholder="请输入中文名称(显示)" />)}
            </FormItem>
            <FormItem label="排序优先级" {...formItemLayout}>
              {getFieldDecorator('sort', {
                rules: [{ required: true, message: '请输入排序优先级' }],
            ...
            <FormItem label="请求路径" {...formItemLayout}>
              {getFieldDecorator('type', {})(<Input placeholder="请输入节点类型" />)}
            </FormItem>
