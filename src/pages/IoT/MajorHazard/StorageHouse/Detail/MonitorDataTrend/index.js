...
export default class MonitorDataTrend extends Component {
  ...
  componentDidUpdate({ majorHazardMonitor: { storageHouseDetail: { id: prevId }={} } }) {
    const { majorHazardMonitor: { storageHouseDetail: { id }={} } } = this.props;
    if (prevId !== id) {
      this.setState({
   ...
   */
  getMonitorDataTrend() {
    const { majorHazardMonitor: { storageHouseDetail: { id }={} }, getMonitorDataTrend } = this.props;
  ...
  render() {
    ...
    const paramList2 = Array.isArray(storageHouseMonitorDataTrend) ? storageHouseMonitorDataTrend : [];
