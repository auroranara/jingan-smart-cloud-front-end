...
export default class MonitorDataTrend extends Component {
  ...
  componentDidUpdate({ majorHazardMonitor: { storageAreaDetail: { pointList: prevPointList }={} } }) {
    const { majorHazardMonitor: { storageAreaDetail: { pointList }={} } } = this.props;
    if (prevPointList !== pointList) {
      ...
      }, pointId && this.getMonitorDataTrend);
    }
  }
   ...
   */
  getMonitorDataTrend() {
    const { getMonitorDataTrend } = this.props;
  ...
  render() {
    ...
    const paramList2 = Array.isArray(storageAreaMonitorDataTrend) ? storageAreaMonitorDataTrend : [];
    const { history, unit, normalUpper, largeUpper, normalLower, smallLower, maxValue, minValue } = paramList2.find(({ id }) => id === paramId) || {};
    ...
    return (
      ...
      >
        <Spin spinning={!!loadingMonitorDataTrend}>
          {pointList2.length && history && history.length ? (
            <Fragment>
              <ReactEcharts
                option={option}
              />
