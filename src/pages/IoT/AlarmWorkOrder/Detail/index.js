...
export default class AlarmWorkOrderDetail extends Component {
  ...
  showVideo = () => {
    const { detail: { videoList = [] } = {} } = this.props;
    ...
    return (
      <Card className={styles.card} title="工单信息">
        <DescriptionList className={styles.descriptionList} gutter={24}>
          ...
          {isHost &&
            !isFinished && <Description term="消防主机">{deviceName || <EmptyData />}</Description>}
          {isHost &&
            !isFinished && (
              <Description term="部件类型">{unitTypeName || <EmptyData />}</Description>
            )}
        </DescriptionList>
        <DescriptionList className={styles.descriptionList} gutter={24}>
          {isHost &&
          ...
          <Description term="监测类型">{reportTypeName || <EmptyData />}</Description>
  ...
  renderImagePreview() {
    ...
    return <ImagePreview images={images} currentImage={currentImage} />;
  }
