    ...
    return (
      <Section title="维保统计">
        <div className={styles.top}>
          ...
          {workOrderList1.length === 0 ? '--' : <Progress width={100} type="circle" percent={percent} strokeColor="#04fdff" status="active" format={percent => <span style={{ color: '#04fdff' }}>{percent + '%'}</span>} />}
        </div>
        <div className={styles.bottom}>
          <div className={styles.itemWrapper}>
            <div className={styles.item} onClick={()=>{handleShowOrder('workOrder', 7);}}>
              <div className={styles.itemValue} style={{ color: '#ff4848' }}>{workOrderList7.length}</div>
              <div className={styles.itemLabel}>已超期工单</div>
            </div>
          </div>
          <div className={styles.itemWrapper}>
            <div className={styles.item} onClick={()=>{handleShowOrder('workOrder', 2);}}>
              <div className={styles.itemValue} style={{ color: '#04fdff' }}>{workOrderList2.length}</div>
              <div className={styles.itemLabel}>待完成工单</div>
            </div>
          </div>
          <div className={styles.itemWrapper}>
            <div className={styles.item} onClick={()=>{handleShowOrder('workOrder', 1);}}>
              <div className={styles.itemValue} style={{ color: '#04fdff' }}>{workOrderList1.length}</div>
