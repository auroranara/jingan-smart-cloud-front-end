import leftLine from './leftLine.png';
import line from './line.png';
import noChecking from './noChecking.png';
import noDanger from './noDanger.png';
import noData from './noData.png';
import noMonitor from './no-monitor.png';
import normal from './normal.png';
import normalCheck from './normalCheck.png';
import normalCheckPoint from './normalCheckPoint.png';
import numberBg from './number-bg.png';
import over from './over.png';
import overTime from './overTime.png';
import play from '../img/play.png';
import pointDot from './pointDot.png';
import pointError from './pointError.png';
import pointNormal from './PointNormal.png';
import pondAbnormal from './pond-abnormal.png';
import pondLoss from './pond-loss.png';
import pondNormal from './pond-normal.png';
import ponit from './ponit.png';
import rightLine from './rightLine.png';
import tempAlarm from './temp-alarm.png';
import tempLost from './temp-lost.png';
import tempNormal from './temp-normal.png';
import titleBg from './title-bg.png';
import titleBgActive from './title-bg-active.png';
import videoDot from './videoDot.png';
import voltAlarm from './volt-alarm.png';
import voltLost from './volt-lost.png';
import voltNormal from './volt-normal.png';
import waitCheckPoint from './waitCheckPoint.png';
import waitTime from './waitTime.png';
import waterBg from './waterBg.png';

export {
  leftLine,
  line,
  noChecking,
  noDanger,
  noData,
  noMonitor,
  normal,
  normalCheck,
  normalCheckPoint,
  numberBg,
  over,
  overTime,
  play,
  pointDot,
  pointError,
  pointNormal,
  pondAbnormal,
  pondLoss,
  pondNormal,
  ponit,
  rightLine,
  tempAlarm,
  tempLost,
  tempNormal,
  titleBg,
  titleBgActive,
  videoDot,
  voltAlarm,
  voltLost,
  voltNormal,
  waitCheckPoint,
  waitTime,
  waterBg,
}
