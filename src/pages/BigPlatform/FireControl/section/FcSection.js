...
export default class FcSection extends PureComponent {
  render() {
    const { title, backTitle, handleBack=null, children=null, style={}, isBack=false, ...restProps } = this.props;
    const newStyle = {
      padding: '0 15px',
      overflow: 'hidden',
      height: '100%',
        ...
        {title
          ? (
            <h3 className={styles.title}>
              ...
              {title}
              {backTitle && <span className={styles.back} onClick={handleBack}>{backTitle}<Icon type="double-right" style={{ marginLeft: 3 }} /></span>}
            </h3>
          ) : null}
         {children}
      </div>
