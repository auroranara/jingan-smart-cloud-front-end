    ...
    if (image) {
      ...
      return (
        <div
          ...
          style={{
            position: 'absolute',
            left: `calc(${positionX/skewX*100}% + ${offsetX}px)`,
            top: `calc(${positionY/skewY*100}% + ${offsetY}px)`,
            minWidth: '233px',
            padding: '20px',
        ...
        >
          <div style={{ background: `url(${background}) no-repeat center / 100% 100%` }}>
            <div style={{ padding: '10px', background: 'rgba(255,255,255,0.5)' }}>
              {data.map(item => {
                return (
                  <div style={{ fontSize: 0 }} key={item[icon]}>
                    <Avatar src={item[icon]} size="small" style={{ verticalAlign: 'top' }} />
                    <div style={{ display: 'inline-block', marginLeft: '8px', verticalAlign: 'top', height: '24px', lineHeight: '24px', fontSize: "14px", color: '#fff' }}>{item[render] ? item[render](item[title]) : item[title]}</div>
                  </div>
                );
              })}
            </div>
